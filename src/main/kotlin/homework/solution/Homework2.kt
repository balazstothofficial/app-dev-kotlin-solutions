...
import homework.solution.Discount.Bundle
import homework.solution.Discount.Fixed
import homework.solution.Discount.Percentage
...
fun main() {
    ...
    val goldenFig = Item("goldenFig")

    val shop = Shop(
        prices = mapOf(banana to 90u.cents, apple to 60u.cents, goldenFig to 16u.euro)
    )

    val cart = Cart(shop = shop) +
            (banana to 3u) +
            (apple to 7u) +
            (goldenFig to 2u) +
            Percentage(20u) +
